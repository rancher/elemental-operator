...
type E2EConfig struct {
	...
	DefaultsChart    string `yaml:"defaultsChart"`
...
func ReadE2EConfig(configPath string) (*E2EConfig, error) { //nolint:gocyclo
	...
	if chart := os.Getenv("CHART"); chart != "" {
		...
		chartFile = strings.Replace(chartFile, "elemental-operator", "elemental-operator-defaults", 1)
		config.DefaultsChart = filepath.Join(dir, chartFile)
	}
